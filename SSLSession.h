...
/** Class to manage a SSL TCP/IP session.
 *
 *  The SSLSession class manages a SSL TCP/IP session.  This includes;
 *  providing staging (either memory buffers or files) to hold data
 *  either to be read from or written to the socket (witin the SSLConn
 ...
 *
 *  The SSLSession class *should be* thread safe, i.e., any instance,
 ...
 *  buffers (and meta-data) require a MUTEX lock within the SSLSession
 *  class.  There is a separate lock for incoming and outgoing
 *  operations.
 *
 *  RCSID: $Id: SSLSession.h,v 1.3 2014/04/11 17:42:15 akadams Exp $
 *
 *  @see SSLConn
 *  @see MsgHdr
 *
 ...
 */
class SSLSession : public SSLConn {
 public:
  /** Constructor.
   *
   */
  explicit SSLSession(const uint8_t framing_type);

  /** Destructor.
   *
   */
  virtual ~SSLSession(void);

  /** Copy constructor, needed for STL.
   *
   */
  SSLSession(const SSLSession& src);

#if 0  // XXX
  /** Equality operator, needed for STL.
   *
   */
  int operator ==(const SSLSession& other) const;
#endif
  ...
  */
  ...
  /** Routine to initialize the internal memory buffers within a SSLSession.
   *
 ...
 protected:
  // Data members.
  uint8_t framing_type_;        // framing type of SSL session (@see MsgHdr)
 ...
 private:
  ...
  // are only called internally by public SSLSession routines that
